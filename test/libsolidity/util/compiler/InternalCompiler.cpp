...
)
{
    ...
    for (auto sourceName: m_stack.sourceNames())
    {
        ...
        {
            std::string const& contractName = contract->fullyQualifiedName();
            LinkerObject const& object = m_stack.object(contractName);
            LinkerObject const& runtimeObject = m_stack.runtimeObject(contractName);
            ...
            {
                if (
                	auto const* items = m_stack.assemblyItems(contractName);
                 	items != nullptr
                )
                    assemblyItems = std::make_optional(*items);

                if (
                	auto items = m_stack.runtimeAssemblyItems(contractName);
                 	items != nullptr
                )
