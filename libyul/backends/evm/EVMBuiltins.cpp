...
EVMBuiltins::EVMBuiltins()
{
	for (auto const& [name, opcode]: evmasm::c_instructions)
	{
		...
		if (opcode == evmasm::Instruction::PREVRANDAO && name == "DIFFICULTY")
			m_functions.emplace_back(instruction, instructionBuiltin(opcode, langutil::EVMVersion::london()));
		else
			m_functions.emplace_back(instruction, instructionBuiltin(opcode, langutil::EVMVersion::current()));
		...
		if (
			...
			std::get<0>(m_functions.back()) |= replaced;
	}

	m_functions.emplace_back(objectAccess, linkersymbolBuiltin());
	m_functions.emplace_back(objectAccess, memoryguardBuiltin());

	m_functions.emplace_back(objectAccess | requiresNonEOF, datasizeBuiltin());
	m_functions.emplace_back(objectAccess | requiresNonEOF, dataoffsetBuiltin());
	m_functions.emplace_back(objectAccess | requiresNonEOF, datacopyBuiltin());
	m_functions.emplace_back(objectAccess | requiresNonEOF, setimmutableBuiltin());
	m_functions.emplace_back(objectAccess | requiresNonEOF, loadimmutableBuiltin());

	m_functions.emplace_back(objectAccess | requiresEOF, auxdataloadnBuiltin());
	...
	yulAssert(
		ranges::all_of(
			m_functions,
			[](std::tuple<Scopes, BuiltinFunctionForEVM> const& _builtin)
